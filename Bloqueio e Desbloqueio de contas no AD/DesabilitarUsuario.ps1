$logfile = ".\temp\Desabilitar_output.txt"
...
$opt = 1
$output = ".\temp\Desabilitar_output.txt"
...
write "`n"
Write "######################################`n"
$selecao = Read-Host "Digite o número do colaborador a ser desabilitado: "
write "`n"
$selecao = $selecao - 1
if(($selecao -lt $num) -and ($selecao -ge 0)){
	if($opt -eq 1){
		Disable-ADAccount -Identity $User.SamAccountName[$selecao]
		$output = Get-ADUser -identity $User.SamAccountName[$selecao] -Property Enabled | Where-Object {$_.Enabled -like “false”} | FT Name, Enabled -Autosize
	}else{
	Disable-ADAccount -Identity $User.SamAccountName
		$output = Get-ADUser -identity $User.SamAccountName -Property Enabled | Where-Object {$_.Enabled -like “false”} | FT Name, Enabled -Autosize
	}
	Write $Data $output | Out-File –Append -FilePath $logfile 
    gc $logfile -Tail 7
	Write "Acesso Revogado com sucesso!"
}else{
